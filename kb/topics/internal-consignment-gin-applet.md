...
- 2026-09-05 — Application Settings = shared `FieldConfigurationComponent`. The shell stores the registry code `internalConsignmentGINApplet` as `appletCode` (applet-loader L195); the shared screen's tab map is keyed `internal-consignment-gin-applet` (the dev-only value from the applet's main.ts L14), so on a live tenant the Payment / Department Hdr / Doc Link / Export tab toggles are never rendered (`showPaymentTab` etc. default false; Contra defaults true; Attachments toggle is in an ungated panel). ~40 `HIDE_*` keys plus `PRINTABLE` pass the four proofs; `HIDE_MAIN_PAYMENT_TAB`, `HIDE_DEPARTMENT_HDR_TAB`, `HIDE_DOC_LINK_TAB`, `HIDE_EXPORT_TAB`, `HIDE_CURRENCY` are read without a control; `SHOW_DOCUMENT_DELETE_BUTTON` is read without a model declaration [src:blg-shared-utilities/.../field-configuration.component.ts L60–L87, L2389–L2436] [src:blg-akaun-mf-app-ng/.../applet-loader.component.ts L195] [src:blg-shared-utilities/modules/permission/field-configuration/field-configuration/field-configuration.component.html L1592, L1775, L6136] [src:.../internal-consignment-gin-edit.component.ts L104–L112]
...
- 2026-09-05 — Customer UAT on the shared hide screen: no hide for Payment / Department Hdr tabs, Credit Terms, Currency, Permit No, Tracking ID for this applet code. Explained by the tab-map key mismatch above for Payment / Department Hdr; Permit No and Tracking ID hides are rendered (Doc Settings panel, ungated); Currency and Credit Terms genuinely have none [src:gh:bigledger/blg-int-general-task#2016] [src:.../main-details.component.html]
