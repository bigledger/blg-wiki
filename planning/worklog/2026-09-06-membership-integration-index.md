---
date: 2026-09-06
unpublished_from: content/en/modules/membership/integration/_index.md
url: /modules/membership/integration/
tier: 2
mechanism: rewritten in place
reason: "Described a 'Data Hub Membership API' — a separate membership API with its own OAuth 2.0 and API-key authentication, RBAC, automatic token refresh, event streams, and asynchronous job-status endpoints. None of it exists. It also had no front matter (every key was commented out with `[//]: # (…)`) and linked to /modules/membership/integration/api-documentation, a page that has never existed."
evidence:
  - "There is no 'Data Hub Membership API'. Membership resources sit on the ordinary surface: /core2/tnt/dm/crm/membership-cards/etl-ep, /crm/membership-class/etl-ep, /crm/membership-points-txn/etl-ep."
  - "No OAuth 2.0. The platform accepts exactly two credentials — a 25-character access key presented as AccessId/AccessKey (IdentityAccessKeyService.java:43-45), and an RS256 JWT. There is no authorization-code flow, no scopes and no refresh-token exchange; JwtGenerator.generateJwt defaults to a 30-day token with no refresh path."
  - "No per-key RBAC. An access key resolves to a login subject and inherits that user's tenant permissions wholesale (AccessKeyDatabaseValidator / AccessIdKeyInterceptor). There is no per-key scope."
  - "No event streams. The only outbound mechanism is a single unsigned HTTP POST per subscription, with no retry (WebhookService.java:56-96)."
  - "Front matter was inert: the file opened `[//]: # (---)`, so Hugo saw no title, description or weight."
  - "The `api-documentation` link had no target: `ls content/en/modules/membership/integration/` shows only _index.md, ecommerce-integration.md, pos-integration.md, third-party-integration.md."
salvage: "The two surviving children — pos-integration.md and ecommerce-integration.md — describe in-product flows and were kept. The section needed an index that keeps navigation intact, so the file was rewritten rather than drafted; the original body is preserved verbatim below."
to_rewrite: "Done in the same commit: a short section index pointing at the two children and at /developers/integration/ for the API, with a callout telling anyone who built against the old page where to re-point."
---

Also corrected in the same pass, in `pos-integration.md`: *"API keys used by POS terminals have
restricted permissions limited to member lookup, points operations, and redemption processing"* —
false for the same reason as the RBAC claim above. Replaced with the accurate statement that a key
inherits its user's permissions, so scoping a terminal means issuing its key against a dedicated
user. One sentence, corrected in place rather than unpublishing a 507-line page of otherwise
in-product prose (ADR-0008 tier 3).
