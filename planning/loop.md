...
| 20 | 2026-09-06 | measure | The audit backlog: 498 unchecked pages ranked by reader risk | ADR-0008 keeps tier-3 pages live and queues them; nobody had ordered that queue. `planning/audit-backlog.md` ranks all 498 by **actionability + 4 × inbound links** (real links only — an earlier pass counted substrings and made every section index look critical). Result: **97 highly actionable pages** that tell a reader to do things and have never been checked — that is the real backlog; 118 middling; **283 orientation-only**, which ADR-0008 explicitly protects. Top of the list is `/applets/sales-workflow/external-sales-invoice-applet/` — **3,555 lines, 302 numbered steps**, in a folder the lanes never reached because lane 1 was drained first, and every comparable page the lanes have opened was substantially invented. Also checked and closed: **no other `user-guide/demo/*` page names a real organisation** — the UTM set was unique. And a structural finding: several `modules/` and `modules-v2/` pages are exact duplicates, so the duplication should be resolved before either is audited. |
| 21 | 2026-09-06 | write | The data-hub integration path — the second external audience | Shipped 6378e93b. **"Data-hub" has no name in the product** — it is marketing language for the **`etl-ep` access endpoint**: 1,049 unique verb+path pairs over 287 resources across 156 controllers (the 952 figure was mappings, not paths). New `/developers/integration/` — `_index`, `getting-started` (nothing → moving real data in seven steps) and `data-api` (path grammar, resources, operations, paging, envelopes, limits, error codes). **`authentication.md` and `webhooks.md` rewritten from source and un-drafted.** A **second fabricated API** was found and unpublished — `modules/membership/integration/third-party-integration.md`, 716 lines of endpoints that do not exist plus an HMAC handler whose signature section would make a receiver reject every genuine delivery; it sat under `modules/`, which is why the developers audit missed it. Verdict: **an outsider can now authenticate, discover and read any resource from the wiki alone; writing business documents still stops at the schema** — and unlike the applet path's blockers, that one is closable with a single verified example body. **Fourteen product findings filed (P-0031…P-0044), four of them serious auth defects.** |
| 22 | 2026-09-06 | batch | Lane 4 run 33 merged; **60 UCC screenshots publishing real customer data quarantined** | Shipped 4255cd30. The UCC applet page was publishing **Malaysian mobile numbers, customer and agent names, e-mail addresses and WhatsApp chat transcripts** across 60 of its 71 screenshots — the largest single privacy exposure the applet programme has found. All 60 off the CDN, 11 safe ones kept. T2T Admin and Warranty Admin rewritten; the **Web Hook applet page unpublished** (tier 1 — every quantified claim contradicted: no retries, no signing, no filtering, 53 seeded topics not "200+"). New shape found: **an ACTIVE registry row is not proof source exists** — `t2tAdminApplet` ships a CDN bundle from no repository across 455 org repos — and the built-but-unregistered count is now **six**. Product findings P-0045…P-0050 (a member can self-approve and self-issue their own warranty certificate). Also fixed a duplicate product id caused by two agents writing the register concurrently, and added an id-allocation rule to both READMEs. |
| 23 | 2026-09-06 | measure | **Site-wide sweep for fabricated API documentation outside `developers/`** | The developers audit cleaned `developers/`; a later unit then found a second fabricated API under `modules/` — 716 lines of endpoints that do not exist, with an HMAC section that would make a receiver reject every genuine delivery. It escaped purely by not living in `developers/`. **122 pages outside `developers/` carry endpoint-shaped content**, the obvious cluster being `modules/membership/api-reference/` and its `modules-v2/` twin (files literally named `POST Create Member.md`). Unit measures every one against the real route table, acts under ADR-0008 with a stop-and-report guard rail above five pages, settles whether `modules/` and `modules-v2/` are duplicates, and adds a lint if a mechanical signature exists. Lane 4 run 34 running alongside on the UCC rewrite. |
| 24 | 2026-09-06 | batch | Fabricated-API sweep merged; Lane 4 run 34 (Unified Contact Center) | Shipped 9044f00f and fb36607d. The sweep came back **98% innocent — 119 of 121 pages** — and the cluster I most suspected (`modules*/membership/api-reference/`, files named `POST Create Member.md`) is **genuine**, reading like captured Postman traffic. Three pages down, **18 links in 17 files repaired** that pointed at pages the developers audit had drafted the same morning without meeting the ADR's link-repair guard rail, and **four new lints** (impossible endpoint shapes, non-resolving hostnames, invented auth headers, signed JWTs) — three of which would have caught the fabricated membership API mechanically. **The worst find was not an API problem**: a real RS256 platform JWT with `sysAdminRank: ADMIN` was committed to this **public** repo on 2025-11-26 and expired 2025-12-14 — a valid admin token, publicly readable, for ~18 days. Scrubbed; still in history (P-0062). Verified independently by decoding the payload. UCC rebuilt 873 → 573 lines, and it **overturned our own P-0008**: client-side permissions are not decorative — UCC seeds 67 and they work, so the gaps are specific (13 codes checked with no definition row; five menus gated by the wrong permission) rather than universal. Broken links 176 → 173. |
| 25 | 2026-09-06 | measure | **Translation-orphan sweep** + the top of the audit backlog | The `zh` Web Hook page stayed live promising 自动重试, OAuth and 200+ 事件类型 for hours after the English came down, because the two sit at different paths and same-path checks miss it. Measured: **58 of 103 `zh` pages and 11 of 24 `ms` pages have no same-path English twin**, and ~25 English pages were rewritten or unpublished today — so an unknown number of translations are now publishing content the English no longer says, in languages few reviewers here can spot-check. Unit pairs by **basename**, classifies every orphan, repairs path-pairing, and adds a lint forbidding a published translation whose English twin is drafted. **Told explicitly not to unpublish** — it will exceed ADR-0008's five-page guard rail, so it reports and I make the call. Second unit started on `external-sales-invoice-applet.md`, rank 1 of 498 in the audit backlog: 3,555 lines, 302 numbered steps, never verified, in a folder the lanes never reached. |
| 26 | 2026-09-06 | write | **Audit-backlog rank 1: `external-sales-invoice-applet` — the applet does not exist** | Shipped 90873304. 3,555 lines and 302 numbered steps describing multi-source creation, pricing tiers, SST/VAT/WHT, draft→final workflow, GL posting and inventory reduction. **There is no registry row** — not ACTIVE, DELETED or INACTIVE — and what ships is a read-only listing whose create form has five controls (Branch, Location, "Conversion Code", "Conversion Name", Description): a half-converted Stock Conversion clone with the Lines tab and **every write effect commented out**, so SAVE and FINAL are enabled buttons that do nothing. Also had "External" backwards — it means the document originated outside BigLedger, not that the customer is outside the group. Seventh built-but-unregistered applet. Two systemic finds: **75 AI-generated infographics across 32 folders carry a NotebookLM watermark** and assert unverified behaviour (verified independently by canvas size and by opening one), and **`sales-workflow/` is 30 pages with 20 lacking `applet_code:`** — the folder lane 1 never reached. |
| 27 | 2026-09-06 | batch | **Translation-orphan sweep — 57 pages unpublished on Vincent's decision** | Shipped f09fd996 and b36b7733. Paired all 134 translated pages by **basename**, not path: **58 were publishing content the English no longer says**, and path-orphaning was the smaller half — the larger half were pages that pair perfectly by path and have simply drifted. Included ISO 27001 / SOC 2 claims, invented approval routing the English dropped that morning, and 16 pages whose English original was retired months ago. The **Chinese home page** could not be drafted (language root), so its false claims were removed in place: a **99.9% uptime SLA guarantee**, a free-trial button and four links to a non-existent partners section — none of which the English home claims. 16 faithful pages **repaired** via `translationKey` rather than moved. New lint: **no published translation of an unpublished English page**, proved by flipping the zh webhook page back and watching it fail. Vincent's two format decisions recorded: keep the infographics and **redraw as vector** — mermaid fences (Hextra renders them natively) for flows, inline SVG shortcodes for art-directed diagrams, raster only for screenshots. |
