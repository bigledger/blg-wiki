...
7a. **Lateral pass on the issues you read.** For every GitHub issue you used (or discarded as off-topic for this page), add one line to your ledger record's `lateral` list: `{"issue":"gh:owner/repo#n","touches":["<topic or applet slug>", ...],"note":"<one line: what it implies for that other page/guide>"}`. Off-topic issues are the valuable ones here — an issue about sales returns that reveals an e-Invoice credit-note rule, a stock-take issue that reveals a location-class rule. Anonymised, as always.
