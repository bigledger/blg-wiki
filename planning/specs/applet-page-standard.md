...
4. **Configuration** ← the section this programme adds or completes. Sub-sections:
   ...
   - **Applet settings** — every switch/option in the applet's Settings (or Configuration) screen:
     a table with *Setting · What it controls · Default · Effect when changed · Who can change it*.
     Read from the settings component / config model in the applet repo.
...
6. **Lifecycle and posting** (document applets) — statuses (DRAFT/FINAL/VOID…), allowed
   transitions, what each transition does to stock and to the General Ledger (a small
   Dr/Cr table), and what it triggers (e-Invoice, delivery, pick-pack). From the Java backend.
...
- **Cite in front matter (`sources:`), not in the body.** The body stays clean.
...
- **Do not invent.** If a section cannot be filled from code/issues, write one line: "Not
  configurable in this applet" or "No settings screen" — never a plausible-sounding guess.
