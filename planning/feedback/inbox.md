...
### tests — our link gate has been measuring a fraction of the site (2026-09-07)
- [x] F-0480 (2026-09-07) **`tests/lychee-gate.sh` has never checked most internal links.** Hugo renders internal links as absolute `https://wiki.bigledger.com/…` URLs — measured: **15,134 of 15,905 hrefs in the modules tree alone** — and the gate runs lychee with `--offline`, which skips every http(s) URL. So the figure I have been reporting all week (383 → 159) only ever counted the handful of *relative* links. **The real number, resolved against the built tree, is 271 distinct broken targets across 2,738 instances.** Fixed: `tests/internal-links.sh` resolves every in-site `href`/`src` in `public/` against the built output, gated on `tests/internal-links-baseline.txt` (271), wired into `tests/run-all.sh` and the CI deploy workflow. lychee stays — it still covers relative links and external-URL syntax. **Two lessons worth keeping:** a gate that reports a number is not the same as a gate that measures the thing you care about, and the number being *small* is exactly why nobody questioned it. Also note `public/` left over from a `hugo server` run gives a falsely clean result (it injects `/livereload.js` and a partial tree) — always `hugo --gc --minify --cleanDestinationDir` before measuring.
- [x] F-0481 (2026-09-07) **The shared main menu rendered 13 dead links on every non-English page.** `hugo.yaml` used `pageRef:`, which Hugo resolves per-language — on a Chinese page `pageRef: /modules/core` becomes `/zh/modules/core`, and no Chinese module pages exist. The menu renders on ~145 Chinese pages, so this single config bug accounted for **2,357 of the site's 2,738 broken internal link instances**, and the old lychee gate could not see any of it. Fixed by converting 18 menu `pageRef`s to literal `url:` so every language's menu points at pages that exist; the per-page language switcher is unaffected. **2,738 → 381 instances, 271 → 230 distinct.** Commit cf87b4e7.
- [ ] F-0482 (2026-09-07) The main menu's top entry, **"User Guide", points at `/user-guide-v2/`** — a single 221-line hub page carrying a **"Work in Progress"** callout, an inline `<div style>`, a `<style>` block with ~40 custom CSS rules and a hand-rolled hero gradient. That is the primary entry point to the whole user-facing site, and it violates CLAUDE.md's Hextra rule ("DO NOT use `<div>` tags with inline styles", "no custom CSS classes"). Meanwhile `/user-guide/` exists separately. Decide which is canonical, rebuild it from Hextra components, and point the menu at it. Same class as the `-v2` sections unpublished on 2026-09-06 (F-0421 family) — this one survived because it sits at the top level, not inside `user-guide/`.
- [ ] F-0483 (2026-09-07) The remaining 230 broken targets, now measurable and worth working down: **87 are missing images** (120 instances) — pages referencing files that were never added or were quarantined without the reference being removed; **70 are `/zh/` pages** whose English original moved or was drafted; **25 under `/user-guide/`** including four `/user-guide/{member-listing,item-maintenance,price-book,pricing-scheme}/` pages that several applet pages link to and which do not exist; **26 under `/applets/`**. The images are the cheapest win and the most visible — a broken image renders as a broken icon on the page.
- [x] F-0484 (2026-09-07) **92 references to images that were never created.** `applets/applet-store.md` carried **42 references in English and 42 in Chinese** to files literally named `*_placeholder.svg` — intent markers nobody ever replaced, rendering as broken images through the entire applet-store walkthrough, which is the page a new customer reads first. Removed with their figure captions; eight more non-existent image references removed from four applet pages. **Also a bug in my own checker**: `tests/internal-links.sh` did not URL-decode, so `/images/Purchase-Invoice-No-Stock%20In-applet/…` (a real directory whose name contains a space) read as missing — **47 false positives**. Fixed. Broken internal links **381 → 256 instances, 230 → 142 distinct**. Commit 550226c6.
- [ ] F-0485 (2026-09-07) `applets/applet-store.md` needs a screenshot session, and it is the highest-value one on the backlog: it is the walkthrough a customer follows to install their first applet, and after this cleanup it has **no images at all**. 42 screens were intended. Capture on a clean tenant — the applet store is the one screen family with no customer data on it, so it should be straightforward.
- [ ] F-0486 (2026-09-07) The last **72 broken internal targets are a content backlog, not link rot** — every remaining one is a page that has never existed, linked from pages that assume it does. Three groups. **(1) ~20 `/user-guide/` pages** several applet and membership pages link to: `item-maintenance`, `price-book`, `pricing-scheme`, `member-listing`, `member-class`, `member-label-list`, `group-listing`, `creating-an-item`, `editing-an-item`, `document-item-types`, `introduction`, `navigation`, `platform-overview`, `team`, `implementation`, plus two `industry-solutions/*` pages. **(2) Four applets with no page at all**: `internal-stock-adjustment-applet`, `membership-admin-console-applet`, `process-monitoring-applet`, `supplier-maintenance-applet` — all four are linked from live pages in both English and Chinese, and all four need the ADR-0002 registry check before anyone writes them (`supplier-maintenance` and `membership-admin-console` had Chinese pages retired earlier for documenting screens whose English originals were removed). **(3) Site furniture that was promised and never built**: `/support/contact/`, `/support/troubleshooting/`, `/partners/training/`, `/partners/certification/`, `/developers/applet-sdk/`, `/tutorials/quick-start`, `/best-practices/`, `/demos-resources/webinars/`, `/resources/budgetary/templates/`, and three `/guides/budgetary/*` pages. Group 3 is the honest one to settle first: each is either written or the link is removed — a promise in a nav card is still a promise.
- [x] F-0487 (2026-09-07) **The support centre pointed almost everywhere except somewhere real.** `support/_index.md` offered cards for `/support/troubleshooting/` and `/support/contact/` (neither has ever existed), linked `/partners/training/`, `/partners/certification/` and `/demos-resources/webinars/` (likewise), and sent readers to three hostnames I verified as **NXDOMAIN** — `support.`, `community.` and `status.bigledger.com`. It also published **a named individual's personal Telegram handle** as a support channel, and a **Support Tiers table** committing to 1–2 hour responses and 24/7 coverage — a contractual commitment, the same class the compliance sweep removed, which survived only because it was phrased as "tiers" rather than "SLA". Rewritten around what is real: FAQs, the beginner guide, the user guide, `support@bigledger.com`, and the reader's own BigLedger contact — who can see their tenant when the documentation cannot. Troubleshooting now points at the applet reference pages, which each carry a troubleshooting section keyed to the errors that applet actually produces. Site-wide: **42 references to four non-resolving hosts removed across 37 files** (`forum.bigledger.com` turned out to be dead too), 12 lines carrying the personal handle, and 8 further promised-but-never-built links. New lint covers all of it. Commit f84305d0.
- [ ] F-0488 (2026-09-07) Two things the support rewrite could not settle, both needing Vincent. **(a) Is there a real knowledge base, community forum or status page?** Four subdomains are referenced across the wiki and none resolves — if these exist under other names, they should be linked; if they are planned, they should not be published until they do. **(b) What are the actual support hours and response times?** The removed table claimed 24/7 enterprise coverage. The page now says response times depend on your contract, which is honest but unhelpful — a real published figure would be better if one exists.
- [x] F-0489 (2026-09-07) **14 user-guide slugs were linked without their section segment** — `/user-guide/price-book/` rather than `/user-guide/daily-tasks/price-book/`, and the same for `item-maintenance`, `pricing-scheme`, `member-listing`, `member-class`, `member-label-list`, `group-listing`, `creating-an-item`, `editing-an-item`, `document-item-types`, `introduction`, `navigation`, `platform-overview`, `team`. **Every one of those pages exists**, one level deeper, so these were never missing pages — they were 30 links pointing one level too shallow. Repointed, along with five targets that had genuinely moved. Broken internal links **104 → 63 instances, 60 → 41 distinct**. Commit 58f4aa0e.
- [ ] F-0490 (2026-09-07) **A near-miss worth recording as method, because it would have been hard to spot.** My first attempt at this batch treated `/best-practices/` as a phantom target and removed every line containing that substring. It matched two things it should not have: live links to `/modules/<module>/best-practices/`, which exist; and **the `aliases:` entries in front matter** (`- /modules-v2/claims/best-practices/`) that the modules consolidation had just built — so it would have silently broken the redirects for the whole `modules-v2` migration while the link count went *down*, looking like progress. Caught by diffing before committing and reverted. **The rule: a substring is not a URL.** Deleting lines that merely *contain* a path will hit front matter, aliases and longer paths that legitimately share the prefix. Repoint by anchored pattern; only delete when the whole link has been identified, never by substring.
- [x] F-0491 (2026-09-07) **Four "applets with no page" resolved against the registry — three were real, one was a phantom.** `internal-stock-adjustment-applet` → the live page at `inventory-workflow/stock-adjustment-applet` (registry `erp_stock_adjustment_applet`, ACTIVE); `membership-admin-console-applet` → `membership/membership-admin-applet` (**both** "Membership Admin Console" rows are DELETED — `membershipadminconsole` and `MAC` — while `MEMBERSHIPADMIN` "Membership Admin" is the live one); `supplier-maintenance-applet` → `master-data/supplier-applet-1` (registry `supplier-applet`). **`Process Monitoring Applet` has no registry row under any name** — the nearest matches are `stockLevelMonitoringApplet` and `productionPanningAndMonitoringApplet`, both different applets — so its eight catalogue entries ("Business process tracking and analysis") across English and Chinese were removed as an ADR-0002 phantom. Six Chinese links to applet pages that exist only in English also repointed. Broken internal links **63 → 43 instances, 41 → 32 distinct**. Commit 34d8c705.
- [ ] F-0492 (2026-09-07) Two things this turned up that are worth someone's attention. **(a) `master-data/supplier-applet-1.md`** — the trailing `-1` is a filename collision artefact, not a product name; the registry calls it `Supplier`. The page should be renamed with an alias, which is a small structural change needing the usual spec. **(b) The applet catalogue pages are themselves unaudited** — `applets/_index.md` and `applets/applet-catalog.md` carried the Process Monitoring phantom in both languages, which means neither has ever been checked against the registry despite being the front door to 185 applet pages. They should get the ADR-0002 treatment: every entry checked for an ACTIVE row, and the catalogue regenerated from the registry rather than hand-maintained.
