---
title: Getting Started & Implementation Guides
description: Guided onboarding, role-based learning paths, and comprehensive workflow guides for BigLedger ERP
weight: 25
bookCollapseSection: false
tags:
- user-guide
- onboarding
- learning-paths
---

Your guided entry point to BigLedger. Whether you're logging in for the first time or mastering advanced workflows, start here and follow the path that matches your role.

{{< callout type="info" >}}
**🚀 New to BigLedger?** Jump to the [Quick Start](#-quick-start--your-first-5-steps) section below to get up and running in under an hour.
{{< /callout >}}

## 🚀 Quick Start — Your First 5 Steps

Complete these steps to go from first login to your first transaction:

| Step | What To Do | Guide |
|------|-----------|-------|
| 1️⃣ | **First Login & Orientation** | [Platform Overview](/user-guide/getting-started/) |
| 2️⃣ | **Company & User Setup** | [Administration Guide](/user-guide/administration/) |
| 3️⃣ | **Chart of Accounts** | [Chart of Accounts Setup](/guides/accounting-guides/chart-of-accounts-setup/) |
| 4️⃣ | **First Transaction** | [Standard Sales Workflow](/guides/sales-guides/standard-sales-workflow/) or [Standard Procurement](/guides/purchasing-guides/standard-procurement-workflow/) |
| 5️⃣ | **First Report** | [Financial Reporting Guide](/guides/accounting-guides/financial-reporting/) |

{{< callout type="tip" >}}
**Estimated time:** ~60 minutes to complete all 5 steps. Each step builds on the previous one.
{{< /callout >}}

---

## 🎯 Choose Your Role

Select your role to access guides tailored to your responsibilities:

{{< cards >}}
  {{< card link="/guides/roles/finance-manager/" title="💰 Finance Manager" subtitle="Financial control, reporting & compliance" >}}
  {{< card link="/guides/roles/procurement-manager/" title="🛒 Procurement Manager" subtitle="Purchasing & supply chain management" >}}
  {{< card link="/guides/roles/warehouse-manager/" title="📦 Warehouse Manager" subtitle="Inventory, stock & logistics operations" >}}
  {{< card link="/guides/roles/branch-manager/" title="🏢 Branch Manager" subtitle="Multi-location operations & oversight" >}}
  {{< card link="/guides/roles/cashier/" title="💵 Cashier" subtitle="POS operations & daily cash management" >}}
  {{< card link="/guides/roles/ecommerce-specialist/" title="🌐 E-Commerce Specialist" subtitle="Online sales & marketplace management" >}}
  {{< card link="/guides/roles/merchandising-manager/" title="🏷️ Merchandising Manager" subtitle="Product management & pricing strategies" >}}
  {{< card link="/guides/roles/internal-audit/" title="🔍 Internal Auditor" subtitle="Audit procedures & compliance checking" >}}
{{< /cards >}}

[📋 View all role-based guides →](/guides/roles/)

---

## 📚 Guide Categories

### Core Workflow Guides

{{< cards >}}
  {{< card link="/guides/sales-guides/" title="💼 Sales Workflows" subtitle="7 guides — Standard, Credit, Cash, Instalment, Partial Delivery, Installation, Returns" >}}
  {{< card link="/guides/purchasing-guides/" title="🛒 Purchasing Workflows" subtitle="5 guides — Standard, Direct GRN, Direct Invoice, Invoice-First, Consignment" >}}
  {{< card link="/guides/accounting-guides/" title="📊 Accounting" subtitle="4 guides — Chart of Accounts, Journal Entries, Financial Reporting, Bank Reconciliation" >}}
  {{< card link="/guides/einvoice-guides/" title="📄 E-Invoice & PEPPOL" subtitle="7 guides — setup, pools and routing, validation, the month-end cycle, cancelling an e-invoice, PEPPOL" >}}
  {{< card link="/guides/inventory-guides/" title="📦 Inventory Management" subtitle="Stock management & transfer procedures" >}}
{{< /cards >}}

### Specialized Guides

{{< cards >}}
  {{< card link="/guides/roles/" title="👤 Role-Based Guides" subtitle="8 guides tailored to specific job functions" >}}
  {{< card link="/guides/advanced/" title="🔬 Advanced Guides" subtitle="7 deep-dive guides for ERP specialists and power users" >}}
{{< /cards >}}

---

## 📖 Learning Paths

Follow these structured sequences to master each area. Each path links to existing guides in recommended order.

### 💰 Finance Path

| # | Guide | What You'll Learn |
|---|-------|-------------------|
| 1 | [Chart of Accounts Setup](/guides/accounting-guides/chart-of-accounts-setup/) | Design your account structure |
| 2 | [Journal Entries](/guides/accounting-guides/journal-entries/) | Record financial transactions |
| 3 | [Bank Reconciliation](/guides/accounting-guides/bank-reconciliation-guide/) | Match bank statements to records |
| 4 | [Financial Reporting](/guides/accounting-guides/financial-reporting/) | Generate standard financial statements |
| 5 | [Financial Reporting Excellence](/guides/advanced/financial-reporting-excellence/) | Advanced analytics & custom reports |
| 6 | [Compliance & Audit](/guides/advanced/compliance-audit/) | Regulatory compliance & audit prep |

### 💼 Sales Path

| # | Guide | What You'll Learn |
|---|-------|-------------------|
| 1 | [Standard Sales Workflow](/guides/sales-guides/standard-sales-workflow/) | Complete order-to-delivery flow |
| 2 | [Credit Sales](/guides/sales-guides/credit-sales-workflow/) | Selling on credit terms |
| 3 | [Cash Sales](/guides/sales-guides/cash-sales-workflow/) | Walk-in immediate payment sales |
| 4 | [Instalment Sales](/guides/sales-guides/instalment-sales-workflow/) | Financing agreements (AEON, JCL) |
| 5 | [Partial Delivery](/guides/sales-guides/partial-delivery-workflow/) | Split shipments & multiple deliveries |
| 6 | [Installation Scheduling](/guides/sales-guides/installation-scheduling-workflow/) | Appliances requiring installation |
| 7 | [Returns & Exchanges](/guides/sales-guides/returns-exchanges-workflow/) | Post-sale returns handling |

### 🛒 Procurement Path

| # | Guide | What You'll Learn |
|---|-------|-------------------|
| 1 | [Standard Procurement](/guides/purchasing-guides/standard-procurement-workflow/) | Full requisition-to-payment cycle |
| 2 | [Direct GRN Workflow](/guides/purchasing-guides/direct-grn-workflow/) | Receive goods without PO |
| 3 | [Direct Invoice Workflow](/guides/purchasing-guides/direct-invoice-workflow/) | Invoice-driven purchasing |
| 4 | [Invoice-First Workflow](/guides/purchasing-guides/invoice-first-workflow/) | Process invoice before goods receipt |
| 5 | [Consignment Purchasing](/guides/purchasing-guides/consignment-purchasing/) | Vendor-managed inventory |

### 🏢 Admin & System Path

| # | Guide | What You'll Learn |
|---|-------|-------------------|
| 1 | [Platform Overview](/user-guide/getting-started/) | System orientation & navigation |
| 2 | [Administration](/user-guide/administration/) | Company, users & permissions setup |
| 3 | [E-Invoice Setup](/guides/einvoice-guides/myinvois-setup/) | MyInvois & PEPPOL configuration |
| 4 | [E-Invoice Validation](/guides/einvoice-guides/einvoice-validation/) | Compliance validation rules |
| 5 | [Integration Best Practices](/guides/advanced/integration-best-practices/) | API & system connectivity |
| 6 | [Performance Optimization](/guides/advanced/performance-optimization/) | System tuning & efficiency |

---

## 🔄 Business Process Flows

### Order-to-Cash

1. [Customer Setup](/applets/master-data/customer-maintenance-applet/) →
2. [Sales Order](/guides/sales-guides/standard-sales-workflow/) →
3. [Invoice Generation](/guides/accounting-guides/journal-entries/) →
4. [Payment Collection](/guides/accounting-guides/bank-reconciliation-guide/) →
5. [Bank Reconciliation](/guides/accounting-guides/bank-reconciliation-guide/)

### Procure-to-Pay

1. [Vendor Setup](/applets/master-data/supplier-applet-1/) →
2. [Purchase Order](/guides/purchasing-guides/standard-procurement-workflow/) →
3. [Goods Receipt](/guides/purchasing-guides/direct-grn-workflow/) →
4. [Invoice Matching](/guides/purchasing-guides/invoice-first-workflow/) →
5. [Payment Processing](/guides/accounting-guides/journal-entries/)

### Record-to-Report

1. [Journal Entry](/guides/accounting-guides/journal-entries/) →
2. [Bank Reconciliation](/guides/accounting-guides/bank-reconciliation-guide/) →
3. [Financial Statements](/guides/accounting-guides/financial-reporting/) →
4. [Advanced Reporting](/guides/advanced/financial-reporting-excellence/)

---

{{< callout type="tip" >}}
**🎬 Want to see BigLedger in action?** [Request a personalized demo](/demos-resources/#schedule-a-personalized-demo) — includes a 60-minute live session tailored to your business scenarios, Q&A with experts, and an implementation roadmap.
{{< /callout >}}

---

## Need Help?

- 📧 [Contact Support](mailto:support@bigledger.com) — 24/7 help desk
- 📖 [User Guide](/user-guide/) — Daily operations quick reference
- 🔧 [API Reference](/developers/api-reference/) — Technical documentation
- 📄 [Demos & Resources](/demos-resources/) — Video demos & presentation materials
- 📋 [Content Guidelines](/guides/content-guidelines/) — For documentation contributors

---

*Can't find what you're looking for? [Contact Support](mailto:support@bigledger.com)*