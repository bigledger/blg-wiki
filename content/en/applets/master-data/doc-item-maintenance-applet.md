...
|-----------------|--------------------|
| âœ… Filter BY criteria (pricing scheme, item status, categories) | âŒ Pick specific columns to include in export |
| âœ… Choose which export template/type to use | âŒ Customize the column set for an export type |
| âœ… Export only items matching your filter criteria | âŒ Exclude specific columns from the export |
