...
**Sales Contract Applet user guide still under progress**
...
{{< callout type="info" >}}
**Core Concept**: A **Sales Contract** is more than just a pricing sheet—it is a formalized commitment that defines the **Terms**, **Durations**, and **Legal Templates** governing the business relationship.
{{< /callout >}}
...
{{< cards >}}
  {{< card title="Contract Management" subtitle="Create and track formal customer sales agreements" link="#contract-administration" >}}

  {{< card title="Agreement Templates" subtitle="Use standardized legal formats for consistent commitments" link="#template-configuration" >}}

  {{< card title="Lifecycle Tracking" subtitle="Monitor contracts from initial draft to final expiration" link="#status-lifecycle" >}}

  {{< card title="Detailed Configuration" subtitle="Define specific terms, durations, and service levels" link="#contract-details" >}}

  {{< card title="Status Listing" subtitle="Unified view of all active and pending sales contracts" link="#contract-listings" >}}

  {{< card title="Personalization" subtitle="Tailor dashboard views for sales or legal roles" link="#personalization-settings" >}}
{{< /cards >}}
...
{{< callout type="tip" >}}
**Real-World Example**: A sales rep initiates a new **Sales Contract** for a corporate account. They select the "Annual Maintenance Template," define a **Start Date** of Jan 1st, and an **End Date** of Dec 31st. Once finalized, the system moves it to **Active**, and the finance team uses it as the reference for the year's billing.
{{< /callout >}}
...
│           │
│           └── Renewal/Exit ──→ Contract matures or is extended
```
...
1. **Navigate**: Go to **Sales Contract** and click **"+" (Create)**.
2. **Assign Entity**: Select the **Customer** from the master records.
3. **Choose Template**: Select the appropriate **Agreement Template** (e.g., Standard Terms).
4. **Set Duration**: Enter the **Start Date** and **End Date** for the contract term.
5. **Finalize**: Review the terms and click **Create**. The contract is now logged and searchable in the listing.
...
1. **Access Templates**: Go to the **Agreement Template Container**.
2. **Create/Edit**: Draft a new template with the required legal clauses and standard terms.
3. **Publish**: make the template available for selection by the sales team.
4. **Audit**: Review existing contracts linked to a specific template to ensure compliance.
...
**Q: Can I attach a signed PDF to the sales contract record?**
A: Yes, many setups allow uploading external attachments (like scanned signed copies) to the contract record via the utilities tab to keep all documentation in one place.

**Q: What happens when a contract expires?**
A: The system will automatically update the status to **Expired**. You can use the listing filters to view all contracts expiring within the next 30 days to proactive manage renewals.

**Q: Can I use different templates for different regions?**
A: Absolutely. You can create as many specific templates as needed (e.g., "Malaysia Service Agreement", "Singapore License Agreement") to comply with local legal requirements.

**Q: Does the contract automatically affect pricing in sales orders?**
A: Depending on your integration, active sales contracts can be used to validate pricing or specific terms in downstream sales and invoicing applets.
