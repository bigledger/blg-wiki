...
description: "Process customer refund notes for returned goods or credit adjustments â€” with intercompany support, line item management, and file import/export"
tags:
- sales
- refund-note
- credit-adjustment
...
- **Structured refund creation** â€” Standardized header + line item structure with mandatory fields
- **Reason tracking** â€” Configurable return reason codes for every refund
- **Approval workflows** â€” Multi-level approval before refund processing
- **Intercompany support** â€” Automatic cross-company transaction handling
- **File import/export** â€” Bulk operations via file upload and data export
- **Printable formats** â€” Professional refund note documents for customers
...
{{< callout type="tip" >}}
**Real-World Example**: A customer returns 5 units of Widget-A (RM 50 each) due to defects. The AR clerk creates a Refund Note, adds the line item (5 Ã— RM 50 = RM 250), selects reason "Defective Product", and submits for manager approval. Once approved, the refund posts to the customer's account and the general ledger.
{{< /callout >}}
...
â”‚   â”œâ”€â”€ Select items / enter quantities
â”‚   â”œâ”€â”€ Apply pricing / discounts
â”‚   â””â”€â”€ Assign return reason
â”‚
â”œâ”€â”€ Submit for Approval
â”‚   â””â”€â”€ Routes to configured approver(s)
â”‚
â”œâ”€â”€ Approved
â”‚   â”œâ”€â”€ Posts to customer account
â”‚   â”œâ”€â”€ Creates GL entries
â”‚   â””â”€â”€ Triggers intercompany posting (if applicable)
â”‚
â””â”€â”€ Finalized / Exported
    â”œâ”€â”€ Print refund note document
    â””â”€â”€ Export for reporting
```
...
2. **Create Header**: Click **"+"** â†’ Select **Customer** â†’ Choose **Branch** â†’ Set **Date** â†’ **Create**
3. **Add Line Items**:
...
5. **Submit**: Click **Submit** â†’ Refund note routes to approver
...
| **Customer** | Select the customer receiving the refund | âœ… |
| **Branch** | Originating branch | âœ… |
| **Date** | Refund note date | âœ… |
| **Reference** | External reference number (e.g., original invoice) | Optional |
| **Currency** | Transaction currency | âœ… |
...
**Status Transitions:**
- **Draft** â†’ Edit freely, not yet submitted
- **Submitted** â†’ Sent for approval, no further edits
- **Approved** â†’ Credit posted to customer account
- **Rejected** â†’ Returned to creator with comments
- **Finalized** â†’ Completed and locked
...
| **Amount** | Line total (Qty Ã— Price âˆ’ Discount + Tax) |
...
4. Click **Export** â†’ File downloads to your device
...
6. Confirm import â†’ Refund notes are created as Drafts
...
Access Settings from the gear icon (âš™ï¸) at the bottom of the sidebar.

### Application Settings (Settings â†’ Application Settings)
...
### Default Selection (Settings â†’ Default Selection)
...
### Printable Format Settings (Settings â†’ Printable Format Settings)
...
### Return Reasons Settings (Settings â†’ Return Reasons Settings)
...
### Branch Settings (Settings â†’ Branch Settings)
...
### Workflow Settings (Settings â†’ Workflow Settings)
...
### Default Selection (Personalization â†’ Default Selection)
...
|--------|-------------|-----------|
| **[Sales Invoice Applet](/applets/sales-invoice-applet/)** | Original invoice referenced by refund note | Invoice â†’ Refund Note |
| **[Sales Credit Note Applet](/applets/sales-credit-note-applet/)** | Credit notes for non-return adjustments | Complementary applets |
| **[Receipt Voucher Applet](/applets/receipt-voucher-applet/)** | Refund settlement via payment reversal | Refund Note â†’ Settlement |
| **[Customer Applet](/applets/customer-applet/)** | Customer master data | Customer â†’ Refund Note |
| **[Sales Return Applet](/applets/sales-return-applet/)** | Physical goods return processing | Return â†’ Refund Note |
| **[Chart of Account Applet](/applets/chart-of-account-applet/)** | GL account posting | Refund â†’ GL Entries |

### Module Integration

| Module | Purpose | Data Flow |
|--------|---------|-----------|
| **Sales** | Revenue adjustment and customer credit | Refund â†’ AR Ledger |
| **Finance** | General ledger posting | Refund â†’ GL Journal |
| **Inventory** | Stock-in for returned goods (via Sales Return) | Return â†’ Stock In |
| **Tax** | Tax reversal on refunded amounts | Refund â†’ Tax Adjustment |
...
- **Always reference the original invoice** â€” Link refund notes to the original sales invoice for complete audit trail
- **Use reason codes consistently** â€” Standardize return reason codes across all branches for accurate reporting
- **Review before finalizing** â€” Once finalized, refund notes cannot be edited
- **Process refunds promptly** â€” Delayed refund processing impacts customer satisfaction and aging reports

### Configuration Best Practices
- **Set approval thresholds** â€” Auto-approve small refunds (e.g., below RM 500) to reduce bottleneck
- **Configure printable formats** â€” Professional refund note documents build customer confidence
- **Branch-specific numbering** â€” Use branch prefixes in numbering for easy identification (e.g., KL-RFN-0001)

### Security Best Practices
- **Segregation of duties** â€” Ensure the refund creator and approver are different people
- **Access control** â€” Restrict settings to Owners and Admins only
- **Regular audit** â€” Review refund note reports periodically for unusual patterns
...
### Related Applets
- **[Sales Invoice Applet](/applets/sales-invoice-applet/)** â€” Original sales invoice management
- **[Sales Credit Note Applet](/applets/sales-credit-note-applet/)** â€” Credit note processing
- **[Sales Return Applet](/applets/sales-return-applet/)** â€” Physical goods return handling
- **[Receipt Voucher Applet](/applets/receipt-voucher-applet/)** â€” Payment and refund settlement
- **[Customer Applet](/applets/customer-applet/)** â€” Customer master data management

### Core Documentation
- **[BigLedger User Guide](/user-guide/)** â€” Overall system documentation
- **[Sales Module](/modules/sales/)** â€” Sales module overview
- **[Guides](/guides/)** â€” Getting started and implementation guides
