...
| Workflow / approval | Settings → Workflow Settings links a **company** to a **workflow process** for `INTERNAL_SALES_INVOICE` (fields: Company, Process — both required). Line-level sell-below-price approvals are separate, need no workflow, and are granted in the Approval menu by a user holding `ALLOW_APPROVE_SELL_BELOW_PRICE`. |
...
| Cost of goods sold | A separate journal built from the moving-average cost record: **Dr `COGS` / Cr stock account** (line GL → item-company link → company default; L1195–L1240). Unmapped `COGS` does not throw | `JournalPostingService.java` L1218–L1240 |
...
| Journal posted, but Sales / Output Tax / COGS lines have no GL code | `SALES`, `OUTPUT_TAX` or `COGS` default GL not mapped — these do not throw | Map them, then re-post through support; map all sales defaults before the first FINAL |
