...
description: "Endâ€‘toâ€‘end cashbook reconciliation: define periods, upload bank statements, perform manual/auto matching, and generate auditâ€‘ready reports"
tags:
- bank-reconciliation
- cashbook
- bank-statement
...
{{< callout type="info" >}}
Main tabs within a recon session: Details â†’ Upload Statement â†’ Manual Matching â†’ Auto Matching â†’ Report.
{{< /callout >}}
...
- Settings, Personalization
  - Appâ€‘level configuration and user personalization features (theme, layout, grid presets).

Menu and structure live under `src/app/components/*` and `src/app/models/menu-items.ts`.

## User Roles and Benefits

- Accountant: drives the monthly processâ€”create sessions, upload statements, match, report.
...
The applet exposes configuration under Settings â†’ Application Settings. Toggle visibility of fields/sections for Details, Edit, Department settings, and Cashbook Transaction Lines. Use this to simplify the UI for end users and align with internal controls.
...
{{< figure src="/screenshots/bank-recon/settings-application-2.png" alt="Application Settings â€“ Bank Recon Details toggles (branch, delete, opening/closing balances)" caption="Settings: show/hide branch and balance fields, and other controls in Bank Recon Details." >}}
...
4) Auto Matching
- Oneâ€‘click Run Automatching executes rules to suggest/link matches.
- Review results via Matched Txn Lines and Unmatched Txn Lines.

5) Reporting
- Report 1 (OLD): compact summaryâ€”cashbook closing balance, expected closing per statement, statement closing, and variance.
...
- All reports rely on the same reconciled dataset; outputs are gridâ€‘based and export/print friendly.

6) Listings and Grids (Shared)
- AG Grid with search, filters, column chooser, and pagination.
- Numeric columns are rightâ€‘aligned; amounts show thousands separators with two decimals.
- Dates render as YYYYâ€‘MMâ€‘DD.

## Screens and Workflows

### Create a Bank Recon Session

- Open Bank Reconciliation â†’ click â€œ+â€.
...
- Go to Upload Statement tab â†’ â€œ+â€.
- Pick a Bank format (e.g., Hong Leong Bank variants, Maybank) or â€œDefault Formatâ€.
...
{{< figure src="/screenshots/bank-recon/manual-matching-reconcile.png" alt="Reconcile tab showing cashbook transactions grid" caption="Manual Matching â€“ Reconcile: select candidate lines and reconcile." >}}

{{< figure src="/screenshots/bank-recon/manual-matching-unreconcile.png" alt="Unreconcile tab example with a single matched line" caption="Manual Matching â€“ Unreconcile: reverse earlier matches when necessary." >}}
...
- Review Matched Txn Lines to confirm results; inspect Unmatched Txn Lines for followâ€‘up manual actions.
...
{{< figure src="/screenshots/bank-recon/report1-old.png" alt="Report 1 (OLD) variance summary" caption="Report 1 (OLD): compact summaryâ€”closing balances and variance." >}}
...
- Number formatting: amounts show two decimals with thousands separators; numeric columns are rightâ€‘aligned.
- Date pickers/renderers use YYYYâ€‘MMâ€‘DD.
- Actions (Create/Save/Unreconcile/Reconcile/Run Automatching) are permissionâ€‘aware.

## Demo Script (quick walkthrough)

1) Create Recon
- Bank Reconciliation â†’ â€œ+â€.
- Choose Cashbook, set Opening/Closing dates.
- Enter Opening Balances for Cashbook and Statement; note helper values.
- Save.

2) Upload Statement
- Upload Statement tab â†’ â€œ+â€.
- Select Bank format (or Default) and upload the CSV.
- Confirm it appears in the uploads grid.

3) Manual Matching
- Manual Matching â†’ Reconcile: select lines and reconcile.
- Use Payment Matching detail to review document metadata.
- If needed, Unreconcile to revert.

4) Auto Matching (optional)
- Auto Matching â†’ Run Automatching.
- Review Matched vs Unmatched sets.

5) Reporting
- Report â†’ pick a report â†’ Generate Report.
- Export/print as needed.

## Troubleshooting

- Canâ€™t create: ensure required fields (Cashbook, dates, opening balances) are provided.
- No statement lines: verify you selected the correct Bank format for the CSV.
- Wrong matches: use Unreconcile, adjust filters, and reâ€‘match manually or rerun Auto Matching.
...
Adopt this applet to replace adâ€‘hoc spreadsheets with a consistent, reviewable monthly close process that stands up to audit.
{{< /callout >}}
