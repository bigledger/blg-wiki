...
The canonical running example: **GadgetSphere Sdn Bhd** — a synthetic Malaysian multi-branch consumer electronics retailer sized at the median of the actual customer base. 3 group companies (`GS` retail, `GSO` e-commerce, `GSD` distribution), 22 retail branches across Klang Valley + Penang + JB + Sabah + Sarawak, ~5,200 SKUs across smartphone/computing/gaming/accessory categories, 28 cashbooks, ~85K customer entities, SST-registered, multi-currency. See CLAUDE.md "The running example" section for the full profile. **Voice is direct second-person ("you"), Xero/QuickBooks-style. Do NOT invent a cast of named staff members threaded through every example — Xero/QuickBooks don't do that, and neither do we. GadgetSphere is the backdrop business that the example numbers come from. The reader is "you".** Use generic location codes (`GS-KV-01`, `GS-PEN-01`), category words ("flagship smartphones", "ultraportable laptops"), and "primary bank / secondary bank" instead of specific shopping mall names, brand names, or real bank names — the example is synthetic and must not point to any identifiable real customer.
